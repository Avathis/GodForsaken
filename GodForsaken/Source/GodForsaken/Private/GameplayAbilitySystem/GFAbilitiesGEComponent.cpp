...
#include "GameplayAbilitySystem/GFAbilitiesGEComponent.h"
#include "AbilitySystemLog.h"
#include "AbilitySystemComponent.h"

void UGFAbilitiesGEComponent::GrantAbilities(FActiveGameplayEffectHandle ActiveGEHandle) const
{
	UAbilitySystemComponent* ASC = ActiveGEHandle.GetOwningAbilitySystemComponent();
		...
		const auto TagsToAssign = GrantedAbilityTags.begin();
    	...
    	for (const FGameplayAbilitySpecConfig& AbilityConfig : GrantAbilityConfigs)
    	{
    		...
    		AbilitySpec.DynamicAbilityTags.AddTag(*TagsToAssign);
